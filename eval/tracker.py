...
class Tracker(object):
    ...
    def init_tracks(self, data):
        ...
        self.track_state = {'features': track_feat,
                            ...
                            # 'points' : track_points,
                            # 'pt_features': track_pt_features,
                            # 'nbr_points' : track_nbr_points,
                            'gt_tracking_id': track_gt,
                            'age': track_age
                            }

        # CHANGE: setting class label from openscene embedding
        labelset = [ "a " + label + " in a scene" for label in NOVEL_LABELS]
    ...
    def _update_track_info(self, matches, new_dets, inactive_tracks,
        ...
        for m in matches:
            ...
            base_ids = list(NuScenesClassesBase.values())
            if class_id not in base_ids:
                class_id = MAPPER[torch.argmax(det_embedding[m[1]].half() @ text_features.t()).cpu()]
            # class_id = det_class[m[1]]
            track = {
    ...
    def _update_track_state(self, matches, new_dets, inactive_tracks, track_latent_feat,
                            det_latent_feat, det_boxes, pred_velo, det_class, det_emb, det_gt):
        '''
        ...
        new_track_class = update_field(det_class, track_class, matches, new_dets,
                                      inactive_tracks, update_with_dets=True)
        new_track_embedding = update_feature(det_emb, track_embedding, matches, new_dets,
                                      inactive_tracks, weight=self.feature_update_weight)
        ...
        track_adj_new = graph_util.bev_euclidean_distance_adj(new_track_boxes, None, #new_track_class,
