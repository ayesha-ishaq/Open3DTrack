...
CLASS_LABELS_MAP = {0:'bicycle', 1:'bus', 2:'car',
 3:'motorcycle', 4:'pedestrian', 5:'trailer', 6:'truck' }
...
def generate_nusc_seq_data(nusc, det_boxes, scenes, sequences_by_name, openscene_dir, split, apply_nms=False):
    ...
    for scene_id, scene_name in enumerate(tqdm(scenes)):
        ...
        frame_id = 0
        while True:
            if current_token == "cce9a0212eb1444696e2decbc95a015e":
                continue
            # create_openscene_file(current_token, split)
            # run_bash_script()
            openscene_path = openscene_dir / split / (current_token + ".npz")
            openscene_output = np.load(openscene_path)
            # Get ego pose data
            ...
            for nms_dets_index in range(len(frame_dets_dict['translation'])):
                ...
                # Move them to the ego-pose frame since openscene points are in this frame
                box.translate(-np.array(ego_pose['translation']))
                box.rotate(Quaternion(ego_pose['rotation']).inverse)

                points_mask = points_in_box(box, openscene_output[:, :3].T).T
                points = openscene_output[points_mask][:, :3]
                openscene_feature = openscene_output[points_mask][:, 3:-2]

                frame_det_points.append(points)
                frame_det_pts_features.append(openscene_feature)
                # classes_in_box = openscene_output[points_mask][:, 3]
                # scores_in_box = openscene_output[points_mask][:, 4]

                # unique_values, counts = np.unique(classes_in_box, return_counts=True)
                # class_score_median = []
                # class_score_mean = []
                # class_score_var = []
                # for un_value in unique_values:
                #     class_score_median.append(np.median(scores_in_box[classes_in_box==un_value]))
                #     class_score_mean.append(np.mean(scores_in_box[classes_in_box==un_value]))
                #     class_score_var.append(np.var(scores_in_box[classes_in_box==un_value]))
                
                # weighed_scores = class_score_median * (counts/np.sum(counts))
                # index_label = np.argmax(weighed_scores)
                # label = int(unique_values[index_label])

                # percentage = class_score[index_label]

                # print(unique_values, counts, class_score, weighed_scores, index_label)

                # frame_dets_dict['class'][nms_dets_index] = NuScenesClasses[CLASS_LABELS_MAP[label]]
                # frame_dets_dict['score'][nms_dets_index] = weighed_scores[index_label]
                # frame_dets_dict['score_mean'][nms_dets_index] = class_score_mean[index_label]
                # frame_dets_dict['score_var'][nms_dets_index] = class_score_var[index_label]

            frame_dets_dict['points'] = np.array(frame_det_points).astype(np.float32) # [N, M, 3]
            frame_dets_dict['pt_features'] = np.array(frame_det_pts_features).astype(np.float32) # [N, M, 768]
            ...
            # delete_openscene_file(current_token, split)
...
def generate_nusc_data(version, dataset_dir, detection_dir, output_dir, openscene_dir, apply_nms=False):
    ...
    for split, result_file, scene, output in zip(['train', 'val'], result_files, scenes, output_dirs):
        ...
        data = generate_nusc_seq_data(nusc, det_boxes, scene, sequences_by_name, openscene_dir, split, apply_nms)
...
if __name__ == '__main__':
    ...
    args.add_argument('--openscene_dir', default=None, type=str,
                      help='Directory where processed openscene results are stored')
    ...
    generate_nusc_data(version=args.version,
                       ...
                       openscene_dir=Path(args.openscene_dir),
